...
df_train = pandas.read_csv('../input/train.csv', encoding="ISO-8859-1")[:1000] #upandasate here
df_test = pandas.read_csv('../input/test.csv', encoding="ISO-8859-1")[:1000] #upandasate here
df_pro_desc = pandas.read_csv('../input/product_descriptions.csv')[:1000] #upandasate here
df_attr = pandas.read_csv('../input/attributes.csv')
df_brand = df_attr[df_attr.name == "MFG Brand Name"][["product_uid", "value"]].rename(columns={"value": "brand"})
num_train = df_train.shape[0]
df_all = pandas.concat((df_train, df_test), axis=0, ignore_index=True)
df_all = pandas.merge(df_all, df_pro_desc, how='left', on='product_uid')
df_all = pandas.merge(df_all, df_brand, how='left', on='product_uid')
...
df_brand = pandas.unique(df_all.brand.ravel())
d={}
i = 1000
for s in df_brand:
    d[s]=i
...
#df_all = pandas.read_csv('df_all.csv', encoding="ISO-8859-1", index_col=0)
...
pandas.DataFrame({"id": id_test, "relevance": y_pred}).to_csv('submission.csv',index=False)
print("--- Training & Testing: %s minutes ---" % round(((time.time() - start_time)/60),2))

from sklearn.feature_extraction import text
import nltk

df_outliers = pandas.read_csv('df_all.csv', encoding="ISO-8859-1", index_col=0)
...
ds2 = pandas.DataFrame.from_dict(d,orient='index')
